...
export function computeEstimatedRoi(raw = {}) {
  ...
  const overageRunsPerMonth =
    pricing.runsCap == null ? 0 : Math.max(0, pipelineRunsPerMonth - pricing.runsCap);
  const annualOverage = roundMoney(overageRunsPerMonth * pricing.overagePerRun * 12);
  const annualAgentSpend = 0;
  const totalAnnualCost = roundMoney(annualSubscription + annualOverage + annualAgentSpend);
  ...
  const paybackMonths = monthlySavings > 0 ? roundRatio(totalAnnualCost / monthlySavings) : null;
