...
async function bootstrapGitCredentials(): Promise<void> {
  initIntakeDb();
  await restoreGitCredentialsFromPostgres().catch((err) => {
    logger.warn({ err }, "codebase worker git credential restore failed");
  });
  try {
    loadGitCredentialsFromStore();
  } catch {
...
async function main(): Promise<void> {
  await bootstrapGitCredentials();

  const codebaseWorker = new Worker<
  ...
  logger.info("codebase worker running");
}
